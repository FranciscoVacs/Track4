    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <span
            ...
            aria-hidden="true"
          />
          <span className="uppercase-label text-[11px]">Resultado de inferencia</span>
        </div>
        <span className="text-[9.5px] uppercase tracking-wider font-semibold text-[var(--muted-foreground)]">
        ...
        <AnimatePresence>
          {showBarcode && scenario?.hasBarcode && (
            ...
            >
              ...
              <motion.div
                className="mt-2 inline-block px-2.5 py-1.5 rounded-md bg-white shadow-sm text-[10px] font-semibold text-[var(--lv-navy)] border-l-2"
          ...
          {showBboxes &&
            scenario?.bboxes.map((b, i) => (
              ...
              >
                ...
                />
                <div
                  className="absolute -top-7 left-0 px-2 py-0.5 rounded bg-white shadow-sm text-[10px] font-semibold whitespace-nowrap border-l-2"
                  style={{ borderLeftColor: colorVar(b.color), color: 'var(--lv-navy)' }}
                >
                  {b.label} <span className="tabular-nums opacity-70">· {b.confidence}%</span>
                </div>
              </motion.div>
            ))}
        </AnimatePresence>
      ...
      <div className="mt-5 flex-1 flex flex-col">
        <div className="flex items-baseline justify-between mb-2">
          <span className="text-[10px] uppercase tracking-[0.16em] font-bold text-[var(--muted-foreground)]">
            Confianza
          </span>
          <span className="text-[10px] uppercase tracking-[0.16em] font-bold text-[var(--muted-foreground)]">
            Veredicto
          ...
          <AnimatePresence mode="wait">
            {showVerdict && scenario && v && (
              <motion.div
                ...
                className="h-full w-full rounded-lg px-4 flex items-center gap-3 border-l-2"
              ...
              >
                ...
                <span className="font-display font-extrabold text-xs tracking-wider uppercase" style={{ color: v.text }}>
